        ...
        <>
                ...
                <div className="font-bold text-md lg:flex-grow">
                    {navItems.map(navItem => {
                        return (
                            <Link key={navItem.name} href={navItem.url}><a className={`block px-4 py-2 mt-4 mr-2 duration-200 ${router.pathname === navItem.url ? "border-2 lg:border-0 lg:border-b-2 border-primary-focus rounded-md lg:rounded-none" : ""} ease-in-out hover:bg-opacity-50 hover:scale-105 lg:inline-block lg:mt-0`}>{navItem.name}</a></Link>
                        )
                    })}
                </div>
                <div className="relative mx-auto text-gray-600 lg:block">
                    <input className={`h-10 pl-4 pr-8 text-base md:text-sm focus:text-base duration-200 ease-in-out bg-transparent border-none focus:outline-none focus:ring-base-content focus:ring-1 focus:rounded-sm focus:${(scrollY > 1)? "text-black": text}`} type="search" name="search" placeholder="Search" />
