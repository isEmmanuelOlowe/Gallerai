...
export default function Navbar({textColour}: props) {
    ...
    return (
        <>
            <nav className={`z-50 sticky top-0 flex flex-wrap items-center justify-between min-w-full py-4 border-none first-letter:fixed lg:px-12 ${(scrollY > 1)?`bg-black text-white border-solid border-gray-700 shadow`:`bg-transparent ${text} border-none`}`}>
                ...
                <div className="block lg:hidden" onClick={() => setExpanded(expanded === "hidden"? "": "hidden")}>
                    <button id="nav" className={"flex items-center px-3 py-2 border-2 rounded " + ((scrollY > 1)? "border-white": "border-black")}>
                ...
                <div className="relative mx-auto text-gray-600 lg:block">
                    <input className={`h-10 pl-4 pr-8 text-sm bg-transparent border-none focus:outline-none focus:ring-black focus:ring-2 focus:rounded-full focus:${(scrollY > 1)? "text-white": text}`} type="search" name="search" placeholder="Search" />
